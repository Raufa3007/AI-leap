...
CREATE INDEX idx_pr_number ON public.purchase_requisition(pr_number);
CREATE INDEX idx_pr_status ON public.purchase_requisition(pr_status);
CREATE INDEX idx_department ON public.purchase_requisition(department);
CREATE INDEX idx_created_at ON public.purchase_requisition(created_at);
CREATE INDEX idx_requested_date ON public.purchase_requisition(requested_date);

-- Enable Row Level Security
ALTER TABLE public.purchase_requisition ENABLE ROW LEVEL SECURITY;

-- Create RLS Policy - Allow all operations for all users (matching your existing policy)

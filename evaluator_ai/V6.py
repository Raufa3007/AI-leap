...
@app.route('/default_files', methods=['GET'])
def get_default_files():
    try:
        ...
        for f in sorted(os.listdir(UPLOAD_FOLDER)):
            if f.startswith('rfp_'):
                rfp_name = f
            elif f.startswith('proposal_'):
                proposal_names.append(f)

        return jsonify({'rfp': rfp_name, 'proposals': proposal_names}), 200
    except Exception as e:
